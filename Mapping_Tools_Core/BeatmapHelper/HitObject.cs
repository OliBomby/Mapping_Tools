    ...
    public class HitObject : ITextLine, IComparable<HitObject> {
        ...
        public void SetLine(string line) {
            ...
            if (IsSlider) {
                ...
                if (values.Length > 9) {
                    var split = values[9].Split(new[] {'|'});
                    for (var i = 0; i < Math.Min(split.Length, Repeat + 1); i++) {
                        EdgeSampleSets.Add(InputParsers.TryParseInt(split[i].Split(new[] {':'})[0], out var ess)
                            ? (SampleSet) ess
                            : SampleSet.Auto);
                        EdgeAdditionSets.Add(InputParsers.TryParseInt(split[i].Split(new[] {':'})[1], out var eas)
                            ? (SampleSet) eas
                            : SampleSet.Auto);
                    }
                }
        ...
        public string GetLine() {
            ...
            if (IsSlider) {
                ...
                if (SliderExtras) {
                    ...
                    values.Add(string.Join("|", EdgeHitsounds.Select<int, string>(p => p.ToInvariant())));
        ...
        /// </summary>
        /// <exception cref="System.InvalidOperationException">If this hit object has no type.</exception>
