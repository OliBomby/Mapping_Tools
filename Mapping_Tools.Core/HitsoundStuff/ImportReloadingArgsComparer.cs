    ...
    public bool Equals(ImportReloadingArgs? x, ImportReloadingArgs? y)
    {
        ...
        switch (x.ImportType)
        {
            case ImportType.Stack:
                return x.Path == y.Path && x.X == y.X && x.Y == y.Y;
            case ImportType.Hitsounds:
                return x.Path == y.Path;
            case ImportType.MIDI:
                return x.Path == y.Path && x.LengthRoughness == y.LengthRoughness && x.VelocityRoughness == y.VelocityRoughness && x.Offset == y.Offset;
