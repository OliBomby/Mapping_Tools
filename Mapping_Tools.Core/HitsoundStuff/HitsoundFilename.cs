...
public static class HitsoundFilename
{
    ...
    public static SampleSet GetSampleSet(string filename)
    {
        ...
        if (split.Length < 1) return SampleSet.Soft;
    ...
    {
        string[] split = filename.Split('-');
        if (split.Length < 2) return Hitsound.Normal;

        string hitsound = split[1];
        if (hitsound.Contains("hitwhistle")) return Hitsound.Whistle;

        if (hitsound.Contains("hitfinish")) return Hitsound.Finish;

        if (hitsound.Contains("hitclap")) return Hitsound.Clap;
    ...
    public static int GetIndex(string filename)
    {
        ...
        if (!string.IsNullOrEmpty(remainder)) FileFormatHelper.TryParseInt(remainder, out index);
