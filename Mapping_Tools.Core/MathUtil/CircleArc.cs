...
public struct CircleArc : IEquatable<CircleArc>
{
    ...
    /// </summary>
    public Matrix2 Rotator => new Matrix2(new Vector2(Math.Cos(ThetaStart), -Math.Sin(ThetaStart) * Dir),
                                  new Vector2(Math.Sin(ThetaStart), Math.Cos(ThetaStart) * Dir))
                              * Radius;
    ...
    public List<Vector2> Interpolate(int amountPoints)
    {
        ...
        for (int i = 0; i < amountPoints; ++i)
        {
            ...
            var o = new Vector2(Math.Cos(theta), Math.Sin(theta)) * Radius;
            output.Add(Centre + o);
        }
    ...
    public static bool operator ==(CircleArc left, CircleArc right) => left.Equals(right);
    ...
    {
        if (!(obj is CircleArc)) return false;
    ...
    {
        return
            Centre == other.Centre && Radius == other.Radius && ThetaStart == other.ThetaStart && ThetaRange == other.ThetaRange && Dir == other.Dir && Stable == other.Stable;
    }
