...
public struct BezierCurve
{
    ...
    {
        if (points == null) throw new ArgumentNullException("points", "Must point to a valid list of Vector2 structures.");
    ...
    public static Vector2 CalculatePoint(IList<Vector2> points, double t, double parallel)
    {
        ...
        {
            temp = MathHelper.BinomialCoefficient(points.Count - 1, i) * Math.Pow(t, i) * Math.Pow(c, points.Count - 1 - i);
    ...
    private static Vector2 CalculatePointOfDerivative(IList<Vector2> points, double t)
    {
        ...
        foreach (var pt in points)
        {
            temp = MathHelper.BinomialCoefficient(points.Count - 2, i) * Math.Pow(t, i) * Math.Pow(c, points.Count - 2 - i);
