...
namespace Mapping_Tools.Core.SystemTools;
...
public class TypeConverters
{
    ...
    /// </summary>
    /// <param name="str">
    ///     An expression accepted by <see cref="DataTable.Compute(string, string)" />; commas are treated as
    ///     decimal points.
    /// </param>
    ...
    ///     The timestamp contains more than day, hour, minute, second, and millisecond
    ///     components.
    /// </exception>
    public static TimeSpan ParseOsuTimestamp(string str)
    {
        ...
        for (int i = 0; i < split.Length; i++)
        {
            ...
            string trimmedString = match.Value;

            int intValue = int.Parse(trimmedString, CultureInfo.InvariantCulture);

            // Invert the index so 0 is the rightmost time value
            int pos = split.Length - 1 - i;
            switch (pos)
            {
                case 0:
