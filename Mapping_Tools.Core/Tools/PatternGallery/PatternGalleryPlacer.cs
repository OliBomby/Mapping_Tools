    ...
    public class PatternGalleryPlacer {
        ...
        private void PreparePattern(Beatmap patternBeatmap, Beatmap beatmap, out List<Part> parts, out List<TimingPointChange> timingPointsChanges) {
            ...
            timingPointsChanges = transformNewTiming.Redlines.Select(tp =>
                new TimingPointChange(tp, mpb: true, meter: true, uninherited: true, omitFirstBarLine: true, fuzziness:Precision.DoubleEpsilon)).ToList();
            ...
            if (includePatternSliderVelocity && (targetMode == GameMode.Taiko || targetMode == GameMode.Mania)) {
                timingPointsChanges.AddRange(svChanges.Select(tp => new TimingPointChange(tp, mpb: true, fuzziness: 0.4)));
            }
            ...
            foreach (HitObject ho in patternBeatmap.HitObjects) {
                if (ho.IsSlider) // SliderVelocity changes
                {
                    ...
                    tp.Offset = ho.Time;
                    tp.MpB = ho.SliderVelocity;
                    timingPointsChanges.Add(new TimingPointChange(tp, mpb: true, fuzziness: 0.4));
                }
