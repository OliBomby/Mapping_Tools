...
/// <summary>
///     Helper class for placing a <see cref="PatternGalleryPattern" /> into a <see cref="Beatmap" />.
/// </summary>
public class PatternGalleryPlacer
{
    /// <summary>Gets or sets the beat divisors considered during timing resnapping.</summary>
    ...
    /// <summary>Gets or sets whether colour-combo skips are repaired.</summary>
    ...
    /// <summary>Gets or sets whether osu! stack offsets are converted to manual positions.</summary>
    ...
    /// <summary>Gets or sets whether pattern hitsounds replace target hitsounds.</summary>
    ...
    /// <summary>Gets or sets whether positions are scaled for the target Circle Size.</summary>
    public bool ScaleToNewCircleSize = false;

    /// <summary>Gets or sets whether object timing is scaled to target timing.</summary>
    public bool ScaleToNewTiming = true;

    /// <summary>Gets or sets whether object timing is snapped to the configured divisors.</summary>
    ...
    /// <param name="protectBeatmapPattern">
    ///     If true, copies the pattern beatmap to prevent the pattern beatmap from being
    ...
    public void PlaceOsuPattern(Beatmap patternBeatmap, Beatmap beatmap, double offset = 0, bool protectBeatmapPattern = true)
    {
        ...
        // Apply the changes
        TimingPointChange.Apply(beatmap.BeatmapTiming, timingPointsChanges);
        ...
        if (FixColourHax) beatmap.FixComboSkip();
    ...
    {
        double endTime = ho.GetEndTime(!beatMode);
    ...
    private static TimingPointChange GetSvChange(HitObject ho)
    {
        ...
        return new TimingPointChange(tp, true, fuzziness: 0.4);
    }
    ...
    private void PreparePattern(Beatmap patternBeatmap, Beatmap beatmap, out List<Part> parts, out List<TimingPointChange> timingPointsChanges)
    {
        double patternStartTime = patternBeatmap.GetHitObjectStartTime();

        var originalTiming = beatmap.BeatmapTiming;
        var patternTiming = patternBeatmap.BeatmapTiming;

        var targetMode = (GameMode)beatmap.General["Mode"].IntValue;
        ...
        var timingOverwriteMode = patternTiming.Redlines.Count > 0
            ? TimingOverwriteMode
            : TimingOverwriteMode.OriginalTimingOnly;

        // Get the scale for custom scale x CS scale
        double csScale = Beatmap.GetHitObjectRadius(originalCircleSize) / Beatmap.GetHitObjectRadius(patternCircleSize);
        ...
        var kiaiToggles = new List<TimingPoint>();
        bool lastKiai = false;
        ...
        foreach (var tp in IncludeKiai ? patternTiming.TimingPoints : originalTiming.TimingPoints)
        ...
        var svChanges = new List<TimingPoint>();
        double lastSV = -100;
        // If not including the SV of the pattern, add the SV of the original map.
        // This has to be done because this part of the original map might get deleted.
        foreach (var tp in includePatternSliderVelocity ? patternTiming.TimingPoints : originalTiming.TimingPoints)
            if (tp.Uninherited)
            {
                lastSV = -100;
            }
        ...
        var patternTimeline = patternBeatmap.GetTimeline();
        var transformOriginalTiming = originalTiming;
        var transformPatternTiming = patternTiming;
        if (scaleToNewTiming)
        {
            ...
            foreach (var ho in patternBeatmap.HitObjects)
            {
                ...
                {
                    var tp = ho.BodyHitsounds[i].Copy();
                    tp.Offset = patternTiming.GetBeatLength(patternStartTime, tp.Offset);
                    ho.BodyHitsounds[i] = tp;
                }
            }

            foreach (var tp in kiaiToggles.Concat(svChanges)) tp.Offset = patternTiming.GetBeatLength(patternStartTime, tp.Offset);
            ...
            foreach (var tp in transformPatternTiming.Redlines) tp.Offset = patternTiming.GetBeatLength(patternStartTime, tp.Offset);
            ...
            foreach (var tp in transformOriginalTiming.TimingPoints) tp.Offset = originalTiming.GetBeatLength(patternStartTime, tp.Offset);
        }

        // Fix SV for the new global SV
        double globalSvFactor = transformOriginalTiming.SliderMultiplier / transformPatternTiming.SliderMultiplier;
        if (FixGlobalSv)
        {
            foreach (var ho in patternBeatmap.HitObjects.Where(o => o.IsSlider)) ho.SliderVelocity *= globalSvFactor;
            foreach (var tp in svChanges) tp.MpB *= globalSvFactor;
        }
        else
        {
            foreach (var ho in patternBeatmap.HitObjects.Where(o => o.IsSlider)) ho.TemporalLength /= globalSvFactor;
        }
        ...
        var newTiming = new Timing(transformOriginalTiming.SliderMultiplier);

        double lastEndTime = double.NegativeInfinity;
        foreach (var part in parts)
        {
            double startTime = part.StartTime;
            double endTime = part.EndTime;
            ...
            {
                if (scaleToNewTiming)
                    foreach (var ho in patternBeatmap.HitObjects.Where(o => o.IsSlider))
                    {
                        double bpmSvFactor = SnapToNewTiming
                            ? transformPatternTiming.GetMpBAtTime(ho.Time) / newTiming.GetMpBAtTime(newTiming.ResnapBeatTime(ho.Time, BeatDivisors))
            ...
            foreach (var ho in part.HitObjects)
            {
                ...
                if (ho.IsSlider)
                {
                    ...
                    if (scaleToNewTiming)
                    {
                        double wantedMsDuration = (newTiming.GetMilliseconds(ho.GetEndTime(false), patternStartTime) - newTiming.GetMilliseconds(ho.Time, patternStartTime))
                                                  / ho.Repeat;
                        double trueMsDuration = newTiming.CalculateSliderTemporalLength(SnapToNewTiming ? newTiming.ResnapBeatTime(ho.Time, BeatDivisors) : ho.Time, ho.PixelLength,
                            ho.SliderVelocity);
                        ho.SliderVelocity /= trueMsDuration / wantedMsDuration;
                    }
                    else
                    {
                        ho.TemporalLength = newTiming.CalculateSliderTemporalLength(SnapToNewTiming ? newTiming.Resnap(ho.Time, BeatDivisors) : ho.Time, ho.PixelLength,
                            ho.SliderVelocity);
                    }
                }
            }
        ...
        var transformNewTiming = newTiming;
        if (scaleToNewTiming)
        {
            ...
            foreach (var ho in patternBeatmap.HitObjects)
            {
                ...
                double msEndTime = newTiming.GetMilliseconds(ho.GetEndTime(false), patternStartTime);
                ...
                foreach (var tp in ho.BodyHitsounds) tp.Offset = newTiming.GetMilliseconds(tp.Offset, patternStartTime);
            ...
            foreach (var tp in kiaiToggles.Concat(svChanges)) tp.Offset = Math.Floor(newTiming.GetMilliseconds(tp.Offset, patternStartTime));
        }
        ...
        if (Math.Abs(spatialScale - 1) > Precision.DOUBLE_EPSILON || Math.Abs(CustomRotate) > Precision.DOUBLE_EPSILON)
        {
            ...
            var transform = Matrix2.Mult(Matrix2.CreateScale(spatialScale), Matrix2.CreateRotation(-CustomRotate));
            ...
            double minX = patternBeatmap.HitObjects.Min(o => o.Pos.X);
            double minY = patternBeatmap.HitObjects.Min(o => o.Pos.Y);
            var offset = new Vector2(Math.Max(-minX, 0), Math.Max(-minY, 0));
        ...
        if (FixStackLeniency)
        {
            ...
            if (ScaleToNewCircleSize) patternBeatmap.Difficulty["CircleSize"].DoubleValue = originalCircleSize;
        ...
        if (SnapToNewTiming)
        {
            ...
            // Resnap Kiai toggles
            foreach (var tp in kiaiToggles) tp.ResnapSelf(transformNewTiming, BeatDivisors);

            // Resnap SliderVelocity changes
            foreach (var tp in svChanges) tp.ResnapSelf(transformNewTiming, BeatDivisors);
        }
        ...
        if (includePatternSliderVelocity && (targetMode == GameMode.Taiko || targetMode == GameMode.Mania))
            timingPointsChanges.AddRange(svChanges.Select(tp => new TimingPointChange(tp, true, fuzziness: 0.4)));

        // Add Kiai toggles
        timingPointsChanges.AddRange(kiaiToggles.Select(tp => new TimingPointChange(tp, kiai: true)));
        ...
        if (includeTimingPointHitsounds)
            foreach (var tlo in patternTimeline.TimelineObjects)
                if (tlo.HasHitsound)
                {
        ...
        patternTiming.Clear();
        TimingPointChange.Apply(patternTiming, timingPointsChanges);
