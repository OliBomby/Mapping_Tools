...
public class HitObject : ITextLine, IComparable<HitObject>
{
    ...
    public List<double> GetAllTloTimes(Timing timing)
    {
        ...
        else if (IsSlider)
        {
            ...
            double sliderTemporalLength = timing.CalculateSliderTemporalLength(Time, PixelLength);

            for (int i = 0; i <= Repeat; i++)
            {
                double time = Math.Floor(Time + sliderTemporalLength * i);
                times.Add(time);
            }
        }
    ...
    public void ResetHitsounds()
    {
        ...
        if (IsSlider)
        {
            for (int i = 0; i < EdgeHitsounds.Count; i++) EdgeHitsounds[i] = 0;
            for (int i = 0; i < EdgeSampleSets.Count; i++) EdgeSampleSets[i] = SampleSet.None;
            for (int i = 0; i < EdgeAdditionSets.Count; i++) EdgeAdditionSets[i] = SampleSet.None;
        }

        foreach (var tlo in TimelineObjects) tlo.ResetHitsounds();
    ...
    public static float QuickCalculateLength(IEnumerable<Vector2> controlPoints)
    {
        ...
        {
            if (lastPoint.HasValue) length += (float)Vector2.Distance(lastPoint.Value, cp);
            lastPoint = cp;
        }
    ...
    /// <param name="useOwnSv">
    ///     Whether to use this object's cached <see cref="SliderVelocity" /> instead of resolving timing
    ///     again.
    /// </param>
    ...
    /// <summary>
    ///     Snaps the object's end using direct end-time snapping when timing changes occur inside a slider, otherwise using
    ///     classic duration snapping.
    /// </summary>
    /// <param name="timing">The beatmap timing model.</param>
    /// <param name="beatDivisors">Permitted fractions of a beat.</param>
