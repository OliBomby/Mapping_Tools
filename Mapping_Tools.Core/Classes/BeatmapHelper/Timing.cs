...
public class Timing : IList<TimingPoint>
{
    ...
    /// <summary>
    ///     The global slider multiplier of a <see cref="Beatmap" />. This is here for convenience sake to calculate absolute
    ///     slider velocities.
    /// </summary>
    ...
    public double GetBeatLength(double startTime, double endTime, bool round = false, IBeatDivisor[] divisors = null)
    {
        ...
        double inc2 = (endTime - lastTime) / lastRedline.MpB;
    ...
    /// <summary>
    ///     Assumes all the redlines are in beat timing and calculates the beat time which is X milliseconds offset for a beat
    ///     time.
    ///     0 beatTime returns originTime.
    /// </summary>
    /// <returns></returns>
    public double WalkMillisecondsInBeatTime(double startBeatTime, double milliseconds)
    {
        ...
        if (milliseconds >= 0)
        {
            var firstRedline = GetRedlineAtTime(startBeatTime);
            var lastRedline = firstRedline;
        ...
        else
        {
            ...
            var redline = startIndex == -1 ? GetFirstTimingPointExtended(true) : redlines[startIndex];
    ...
    public double WalkBeatsInMillisecondTime(double beatTime, double originTime = 0, bool round = false, IBeatDivisor[] divisors = null)
    {
        ...
        if (beatTime >= 0)
        {
            var firstRedline = GetRedlineAtTime(originTime);
            var lastRedline = firstRedline;
    ...
    public static double GetNearestTick(double time, TimingPoint tp, IBeatDivisor beatDivisor)
    {
        ...
        if (remainder < 0.5 * d) return time - remainder;
    ...
    public static double GetNearestMultiple(double duration, double divisor)
    {
        ...
        if (remainder < 0.5 * divisor) return duration - remainder;
    ...
    /// <param name="firstTp">
    ///     Overwrites the timing for anything that happens before the first timing point.
        ...
        TimingPoint tp = null, TimingPoint firstTp = null, bool exactMode = false)
    {
        ...
        foreach (var beatDivisor in beatDivisors)
        {
            double t = GetNearestTick(time, beforeTp, beatDivisor);
        ...
        if (!exactMode && afterTp != null && newTime > beforeTp.Offset + 10 && newTime >= afterTp.Offset - 10) newTime = afterTp.Offset;
        ...
        TimingPoint tp = null, TimingPoint firstTp = null, bool exactMode = false)
    {
        ...
        foreach (var beatDivisor in beatDivisors)
        {
            double t = GetNearestTickBeatTime(time, beforeTp, beatDivisor);
            double d = Math.Abs(time - t);
        ...
        if (!exactMode && afterTp != null && newTime > beforeTp.Offset + 10 / beforeTp.MpB && newTime >= afterTp.Offset - 10 / beforeTp.MpB) newTime = afterTp.Offset;
        return newTime;
    }
        ...
        TimingPoint tp = null, TimingPoint firstTp = null)
    {
        ...
        foreach (var beatDivisor in beatDivisors)
        {
            double nd = GetNearestMultiple(duration, beforeTp.MpB * beatDivisor.GetValue());
            double d = Math.Abs(duration - nd);
    ...
    /// <summary>
    ///     Calculates the snapped time for a given time and makes sure the snapped time remains inside the specified time
    ///     range.
    /// </summary>
    /// <param name="time"></param>
    /// <param name="beatDivisors"></param>
        ...
        TimingPoint firstTp = null)
    {
        ...
        if (afterTp != null && newTime > beforeTp.Offset + 10 && newTime >= afterTp.Offset - 10) newTime = afterTp.Offset;

        // Don't resnap if it would move outside
        if (newTime <= rangeStart + 1 || newTime >= rangeEnd - 1) newTime = time;

        return floor ? Math.Floor(newTime) : newTime;
    }

    /// <summary>
    ///     Calculates the duration of a slider using the slider velocity and milliseconds per beat at a given time, global
    ///     multiplier and the pixel length.
    /// </summary>
    ...
    {
        return length * GetMpBAtTime(time) * (double.IsNaN(sv) ? -100 : MathHelper.Clamp(sv, -1000, -10)) / (-10000 * SliderMultiplier);
    }
    ...
    {
        return length * (double.IsNaN(sv) ? -100 : MathHelper.Clamp(sv, -1000, -10)) / (-10000 * SliderMultiplier);
    }
    ...
    {
        int index = timingPoints.BinarySearch(tp);
        if (index >= 0) timingPoints.RemoveAt(index);
    ...
    public int RemoveAll(Func<TimingPoint, bool> match)
    {
        ...
        foreach (var itemToRemove in itemsToRemove) Remove(itemToRemove);
    ...
    {
        int index = BinarySearchUtil.BinarySearch(timingPoints, time, tp => tp.Offset, BinarySearchUtil.EqualitySelection.Rightmost);
        if (index < 0)
        {
            index = ~index;
        ...
        List<TimingPoint> timingPoints, bool inclusive = true)
    {
        ...
        int endIndex = BinarySearchUtil.BinarySearch(timingPoints, endTime, tp => tp.Offset, BinarySearchUtil.EqualitySelection.Rightmost);
        if (endIndex < 0)
            endIndex = ~endIndex - 1;
