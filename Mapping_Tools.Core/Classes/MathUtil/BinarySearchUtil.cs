...
public class BinarySearchUtil
{
    ...
    /// <summary>
    ///     Finds a value X between the lower and upper bounds such that the check function returns true for X, but not for X +
    ///     epsilon
    /// </summary>
    ...
    /// <param name="midFunc">
    ///     Function which calculates the <see cref="T" /> in the middle of two instances of <see cref="T" />
    /// </param>
    /// <param name="checkFunc">Function which checks the validity of an instance of <see cref="T" /></param>
    /// <returns>
    ///     The last known value for which <paramref name="checkFunc" /> is true, within <paramref name="epsilon" /> of
    ///     the boundary.
    /// </returns>
    ...
    ///     If it cant find something equal it'll return the complement of the index of the first item greater than the search
    ///     term.
    /// </summary>
    ...
    public static int BinarySearch<T, T2>(IReadOnlyList<T> items, T2 searchTerm, Func<T, T2> termFunc, EqualitySelection equalitySelection = EqualitySelection.FirstFound)
    {
        ...
        while (min <= max)
        {
            ...
            var midTerm = termFunc(items[mid]);
