...
public sealed class LayerCollection
{
    ...
    public void SetInceptionLevel(int inceptionLevel)
    {
        if (inceptionLevel < 0)
        {
            throw new ArgumentException("Inception level can't be less than 0.", nameof(inceptionLevel));
        }
    ...
    public IEnumerable<IRelevantObject> GetAllRelevantObjects() =>
        ObjectLayers.SelectMany(layer => layer.Objects.Values.SelectMany(objects => objects));
    ...
    public IEnumerable<IRelevantDrawable> GetAllRelevantDrawables() =>
        GetAllRelevantObjects().OfType<IRelevantDrawable>();

    /// <summary>Gets the root layer.</summary>
    /// <returns>The first layer.</returns>
    public RelevantObjectLayer GetRootLayer() => ObjectLayers[0];
    ...
    public IEnumerable<RelevantHitObject> GetRootRelevantHitObjects() =>
        GetRootLayer().Objects.TryGetValue(typeof(RelevantHitObject), out List<IRelevantObject>? list)
