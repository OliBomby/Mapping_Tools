...
namespace Mapping_Tools.Core.Classes.Graph.Interpolation;
...
public sealed class CustomExtremaAttribute : Attribute
{
    ...
    public CustomExtremaAttribute(double[] extremaPositions) => ExtremaPositions = extremaPositions;
...
public sealed class CustomDerivativeExtremaAttribute : Attribute
{
    ...
    /// <param name="extremaPositions">The positions at which extrema are known.</param>
    public CustomDerivativeExtremaAttribute(double[] extremaPositions) => ExtremaPositions = extremaPositions;
...
public sealed class CustomIntegralExtremaAttribute : Attribute
{
    ...
    /// <param name="extremaPositions">The positions at which extrema are known.</param>
    public CustomIntegralExtremaAttribute(double[] extremaPositions) => ExtremaPositions = extremaPositions;
...
public class CustomInterpolator : IGraphInterpolator
{
    ...
    /// <inheritdoc />
    public double GetInterpolation(double t) => InterpolationFunction(t);
}
...
public static class GraphInterpolatorCatalog
{
    ...
    [
        typeof(Interpolators.SingleCurveInterpolator),
        typeof(Interpolators.SingleCurveInterpolator2),
        typeof(Interpolators.SingleCurveInterpolator3),
        typeof(Interpolators.DoubleCurveInterpolator),
        typeof(Interpolators.DoubleCurveInterpolator2),
        typeof(Interpolators.DoubleCurveInterpolator3),
        typeof(Interpolators.HalfSineInterpolator),
        typeof(Interpolators.WaveInterpolator),
        typeof(Interpolators.ParabolaInterpolator)
    ];

    /// <summary>Gets the interpolator types displayed by the graph context menu.</summary>
    public static IReadOnlyList<Type> GetInterpolators() => BuiltInTypes;
    ...
    public static Type GetInterpolatorByIndex(int index) =>
        index >= 0 && index < BuiltInTypes.Length
            ? BuiltInTypes[index]
            : typeof(Interpolators.SingleCurveInterpolator);
    ...
    {
        ArgumentNullException.ThrowIfNull(type);
        if (!typeof(IGraphInterpolator).IsAssignableFrom(type) || type.IsAbstract)
        {
            throw new ArgumentException("The type is not a concrete graph interpolator.", nameof(type));
        }
    ...
    public static IGraphInterpolator Clone(IGraphInterpolator interpolator)
    {
        ...
        IGraphInterpolator clone = GetInterpolator(interpolator.GetType());
    ...
    {
        ArgumentNullException.ThrowIfNull(interpolator);
        if (interpolator is not IDerivableInterpolator derivable)
        {
            return 1;
        }
