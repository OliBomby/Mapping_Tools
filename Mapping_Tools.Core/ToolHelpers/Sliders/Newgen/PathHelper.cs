...
public static class PathHelper
{
    ...
    {
        if (p1.List is null) throw new ArgumentException(@"Point 1 must be part of a linked list.", nameof(p1));
        if (p1.Next is null) throw new ArgumentException(@"Point 1 must have a successor.", nameof(p1));
    ...
    public static int Subdivide(this LinkedList<PathPoint> path, LinkedListNode<PathPoint> start, LinkedListNode<PathPoint> end, int wantedCount)
    {
        ...
        double dist = end.Value.CumulativeLength - start.Value.CumulativeLength;
        bool tMode = Precision.AlmostEquals(dist, 0);

        if (tMode)
        {
            // Interpolate path points at roughly equal distance intervals
            double wantedGap = (end.Value.T - start.Value.T) / wantedCount;
            ...
            while (p != end)
            {
                ...
                double gap = p.Next!.Value.T - p.Value.T;
                if (Precision.DefinitelyBigger(gap, wantedGap))
                {
                    int pointsToAdd = (int)Math.Ceiling(gap / wantedGap) - 1;
        ...
        else
        {
            // Distance mode
            // Interpolate path points at roughly equal distance intervals
            double wantedGap = dist / wantedCount;
            ...
            while (p != end)
            {
                ...
                double gap = p.Next!.Value.CumulativeLength - p.Value.CumulativeLength;
                if (Precision.DefinitelyBigger(gap, wantedGap))
                {
                    int pointsToAdd = (int)Math.Ceiling(gap / wantedGap) - 1;
                    Interpolate(p, pointsToAdd);
        ...
        LinkedListNode<PathPoint> end, double startTemplateT, double endTemplateT, IEnumerable<double> criticalPoints, out LinkedList<LinkedListNode<PathPoint>> ensuredPoints)
    {
        ...
        foreach (double criticalPoint in criticalPoints.Where(t => t >= startTemplateT && t <= endTemplateT))
            ...
            {
                double t = (criticalPoint - startTemplateT) / (endTemplateT - startTemplateT) * (end.Value.T - start.Value.T) + start.Value.T;
            ...
            {
                double t = (criticalPoint - startTemplateT) / (endTemplateT - startTemplateT) * (end.Value.CumulativeLength - start.Value.CumulativeLength)
                           + start.Value.CumulativeLength;
        ...
        IEnumerable<LinkedListNode<PathPoint>> ensuredPoints)
    {
        ...
        if (previousRed is not null) addedPoints += path.Subdivide(previousRed, start, 2);

        var prev = start;
        if (ensuredPoints is not null)
            foreach (var ensuredPoint in ensuredPoints)
            {
                if (ensuredPoint.Value > prev.Value) addedPoints += path.Subdivide(prev, ensuredPoint, 2);
                prev = ensuredPoint;
            }

        if (end.Value > prev.Value) addedPoints += path.Subdivide(prev, end, 2);

        var nextRed = FindNextRed(end);
        if (nextRed is not null) addedPoints += path.Subdivide(end, nextRed, 2);
    ...
    {
        if (start.Value > end.Value) throw new ArgumentException(@"The end point has to be a points that comes after the starting point in the path.");
    ...
    public static LinkedListNode<PathPoint> FindFirstOccurrence(LinkedListNode<PathPoint> start, double cumLength, double t = double.NaN, double epsilon = Precision.DOUBLE_EPSILON)
    {
        ...
        if (BeforeWantedPoint(start.Value, double.IsNaN(t) ? cumLength - epsilon : cumLength, t - epsilon))
            ...
            while (p is not null)
            {
                if (InsideViableRange(p.Value, cumLength, t, epsilon) && !InsideViableRange(prev.Value, cumLength, t, epsilon)) return p;
                if (!InsideViableRange(prev.Value, cumLength, t, epsilon) && !BeforeWantedPoint(p.Value, cumLength, t) && BeforeWantedPoint(prev.Value, cumLength, t)) return prev;
            ...
            while (p is not null)
            {
                if (!InsideViableRange(p.Value, cumLength, t, epsilon) && InsideViableRange(prev.Value, cumLength, t, epsilon)) return prev;
                if (!InsideViableRange(p.Value, cumLength, t, epsilon) && BeforeWantedPoint(p.Value, cumLength, t) && !BeforeWantedPoint(prev.Value, cumLength, t)) return p;
    ...
    public static LinkedListNode<PathPoint> FindLastOccurrence(LinkedListNode<PathPoint> start, double cumLength, double t = double.NaN, double epsilon = Precision.DOUBLE_EPSILON)
    {
        ...
        if (BeforeWantedPoint(start.Value, double.IsNaN(t) ? cumLength + epsilon : cumLength, t + epsilon))
            ...
            while (p is not null)
            {
                if (InsideViableRange(prev.Value, cumLength, t, epsilon) && !InsideViableRange(p.Value, cumLength, t, epsilon)) return prev;
                if (!InsideViableRange(p.Value, cumLength, t, epsilon) && !BeforeWantedPoint(p.Value, cumLength, t) && BeforeWantedPoint(prev.Value, cumLength, t)) return p;
            ...
            while (p is not null)
            {
                if (!InsideViableRange(prev.Value, cumLength, t, epsilon) && InsideViableRange(p.Value, cumLength, t, epsilon)) return p;
                if (!InsideViableRange(prev.Value, cumLength, t, epsilon) && BeforeWantedPoint(p.Value, cumLength, t) && !BeforeWantedPoint(prev.Value, cumLength, t)) return prev;
        ...
        double epsilon = Precision.DOUBLE_EPSILON)
    {
        ...
        if (InsideViableRange(node.Value, cumLength, t, epsilon) || !BeforeWantedPoint(node.Value, cumLength, t) || node.Next is null) return node;
        ...
        double epsilon = Precision.DOUBLE_EPSILON)
    {
        ...
        if (InsideViableRange(node.Value, cumLength, t, epsilon) || BeforeWantedPoint(node.Value, cumLength, t) || node.Previous is null) return node;
    ...
    public static LinkedListNode<PathPoint>? FindNextRed(LinkedListNode<PathPoint> start)
    {
        ...
        while (current is not null)
        {
            if (current.Value.Red || current.Next is null) return current;
            current = current.Next;
        }
    ...
    public static LinkedListNode<PathPoint>? FindPreviousRed(LinkedListNode<PathPoint> start)
    {
        ...
        while (current is not null)
        {
            if (current.Value.Red || current.Previous is null) return current;
