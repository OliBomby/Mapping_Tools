...
public class PathWithHints
{
    ...
    {
        if (hint.Start.Value >= hint.End.Value) throw new ArgumentException("Hint start must be before end.");

        // Find sorted place in the list
        // The hints are not overlapping so end times are also sorted
        int startIndex = BinarySearchUtil.BinarySearch(reconstructionHints, hint.Start.Value,
        ...
        int endIndex = BinarySearchUtil.BinarySearch(reconstructionHints, hint.End.Value,
    ...
    {
        var hintYieldedRight = hint.Start;
        foreach (var overlap in overlaps)
        {
            ...
            else if (p1 < p3)
            {
                ...
                else
                {
                    ...
                    if (overlap.Layer < hint.Layer) continue;
        ...
        if (hintYieldedRight.Value < hint.End.Value) yield return CutHint(hint, hintYieldedRight, hint.End);
    }
    ...
    {
        double dist = hint.End.Value.CumulativeLength - hint.Start.Value.CumulativeLength;
        double startP;
        double endP;
        if (Precision.AlmostEquals(dist, 0))
        {
            // Use T
            double factor = (hint.EndP - hint.StartP) / (hint.End.Value.T - hint.Start.Value.T);
        ...
        {
            // Use distance
            double factor = (hint.EndP - hint.StartP) / dist;
        ...
        return double.IsNaN(startP) || double.IsNaN(endP) || Precision.AlmostEquals(startP, endP)
            ? null
    ...
    public void RecalculateAndFixHints()
    {
        ...
        for (int i = 0; i < reconstructionHints.Count; i++)
        {
            var hint = reconstructionHints[i];
