...
- `Mapping_Tools.Infrastructure/Audio/NaudioAudioClipMixer.cs` and `Files/PhysicalHitsoundStudioFileSystem.cs` are the only new physical adapters. Existing step-41 `IAudioGenerator`, `IAudioExporter`, `IMidiService`, `AudioPreviewService`, SoundFont renderer, effects, playback, and spectrum boundary are reused; no duplicate decoder or spectrum implementation was added.
