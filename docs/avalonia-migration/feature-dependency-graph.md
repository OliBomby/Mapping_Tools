...
| A2 | Beatmap editing gateway | `EditingSession`, `BeatmapEditingSession`, `StoryboardEditingSession`, `EditorReaderStuff` | Load the correct on-disk/editor version, mutate it, save it, reload the editor, and expose selected objects. | Core document editors plus Application gateway; Windows EditorReader implementation in Infrastructure. |
...
| D1 | Application shell | `MainWindow`, `MainWindowVm`, `ViewCollection`, `StandardView` | Tool discovery, navigation/search/favorites, current view activation, title/scroll behavior, project menu, current-map controls, notifications, window state, and shutdown autosave. | Desktop shell with attributed per-tool registrations discovered from built-in and external plugin assemblies. |
| D2 | Forms and common presentation | `Components/Domain`, `ViewHeaderComponent`, drag/drop controls | Validation, formatting, enum/boolean conversion, headers, list reordering, common inputs, and focus helpers. | Desktop owns presentation validation, formatting, behaviors, and reusable views; Core owns only rules that are genuine domain invariants. |
| D3 | Dialog system | `Components/Dialogs`, `MessageWindow`, feature dialogs | Messages, beatmap import, typed values, sample selection, reflection-driven custom forms, and confirmations. | Desktop-owned interaction contracts plus typed Avalonia dialogs. Do not reproduce the reflection/WPF-control coupling in `CustomDialog`. |
...
| Combo Colour Studio | Defines combo-colour sequences and time-based/single-combo colour points, imports existing colours/colour hax, and saves projects. | C1 colours, C2, C4 combo project, A1–A5, A8, D2–D3, I1. | Custom drag/drop list and color editing; project model currently inherits UI-oriented bindable base. | L |
...
34. **Combo Colour Studio.** Port color models, drag/drop ordering, import, and project persistence.
