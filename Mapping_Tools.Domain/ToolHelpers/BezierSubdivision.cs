...
public class BezierSubdivision(List<Vector2> points, int level = 0, int index = 0) {
    public List<Vector2> Points { get; } = points; // List of bezier control Points
    public int Level { get; } = level; // Depth of subdivision
    public int Index { get; } = index; // Index of subdivision
    public int Order => Points.Count - 1; // Bezier polynomial order

    public BezierSubdivision Copy() {
        return new BezierSubdivision([..Points], Level, Index);
    }
