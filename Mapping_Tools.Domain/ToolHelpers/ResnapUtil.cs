...
public static class ResnapUtil {
    ...
    public static double Resnap(this Timing timing, double time, IEnumerable<IBeatDivisor> beatDivisors, bool floor = true,
        ...
        return floor && !exactMode ? Math.Floor(newTime + Precision.DoubleEpsilon) : newTime;
    }
    ...
    public static double ResnapDuration(this Timing timing, double time, double duration, IEnumerable<IBeatDivisor> beatDivisors, bool floor = true,
        ...
        return floor ? Math.Floor(newDuration + Precision.DoubleEpsilon) : newDuration;
    }
    ...
    /// <param name="floor">Whether to floor the time after snapping.</param>
    /// <param name="tp">The uninherited timing point to snap to. Leave null for automatic selection.</param>
    /// <param name="firstTp">Overwrites the timing for anything that happens before the first timing point.
    ///     You can set this to avoid bad timing when there could be an inherited timing point before the first red line.</param>
    /// <returns>The snapped time.</returns>
    public static double ResnapInRange(this Timing timing, double time, IEnumerable<IBeatDivisor> beatDivisors,
        double startTime, double endTime, bool floor = true, TimingPoint? tp = null, TimingPoint? firstTp = null) {
        TimingPoint beforeTp = tp ?? timing.GetRedlineAtTime(time, firstTp);
        TimingPoint? afterTp = tp == null ? timing.GetRedlineAfterTime(time) : null;
        ...
        return floor ? Math.Floor(newTime + Precision.DoubleEpsilon) : newTime;
    }

    #region HitObject

    public static bool ResnapSelf(this HitObject ho, Timing timing, IEnumerable<IBeatDivisor> beatDivisors, bool floor = true,
        TimingPoint? tp = null, TimingPoint? firstTp = null) {
    ...
    public static bool ResnapEndTimeSmart(this IDuration obj, Timing timing, IEnumerable<IBeatDivisor> beatDivisors,
        bool floor = true, TimingPoint? tp = null, TimingPoint? firstTp = null) {
        if (obj is not IHasStartTime s)
            throw new ArgumentException("Object must implement IHasStartTime", nameof(obj));

        // If there is a redline in the sliderbody then the sliderend gets snapped to a tick of the latest redline
        return timing.Redlines.Any(o => o.Offset <= obj.EndTime + 20 && o.Offset > s.StartTime)
            ? obj.ResnapEndTime(timing, beatDivisors, floor, tp, firstTp)
            : obj.ResnapDuration(s.StartTime, timing, beatDivisors, firstTp);
    }

    public static bool ResnapEndTime(this IDuration obj, Timing timing, IEnumerable<IBeatDivisor> beatDivisors,
        bool floor = true, TimingPoint? tp = null, TimingPoint? firstTp = null) {
    ...
    public static bool ResnapDuration(this IDuration obj, double time, Timing timing, IEnumerable<IBeatDivisor> beatDivisors,
    ...
    public static double GetResnappedTime(this IHasStartTime obj, Timing timing, IEnumerable<IBeatDivisor> beatDivisors,
        bool floor = true, TimingPoint? tp = null, TimingPoint? firstTp = null) {
    ...
    /// <param name="firstTp"></param>
    /// <returns></returns>
    public static bool ResnapSelf(this TimingPoint thisTimingPoint, Timing timing, IEnumerable<IBeatDivisor> beatDivisors,
        bool floor = true, TimingPoint? tp = null, TimingPoint? firstTp = null) {
