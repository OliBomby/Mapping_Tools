...
internal static class ValueConverterHelper
{
    ...
    {
        if (targetType != typeof(string)) throw TypeError(typeof(T), targetType);
    ...
    {
        if (targetType != typeof(T) && targetType != typeof(object)) throw TypeError(typeof(string), targetType);
    }
    ...
    public static InvalidCastException TypeError(
        ...
        Type targetType)
    {
        return new InvalidCastException($"Cannot convert {sourceType.Name} to {targetType.Name}.");
    }
}
