...
public sealed class HitsoundStudioProject
{
    ...
    private static HitsoundLayer CloneLayer(HitsoundLayer layer) => new(
        layer.Name,
        layer.SampleSet,
        layer.Hitsound,
        layer.Priority,
    ...
    private static SampleSchema CloneSchema(SampleSchema source)
    {
        ...
        foreach ((string name, List<SampleGeneratingArgs> samples) in source)
        {
            copy[name] = samples.Select(sample => sample.Copy()).ToList();
        }
...
public interface IAudioClipMixer
{
    ...
    Task<Mapping_Tools.Core.Audio.AudioClip> MixAsync(
        IReadOnlyList<Mapping_Tools.Core.Audio.AudioClip> clips,
        CancellationToken cancellationToken = default);
}
