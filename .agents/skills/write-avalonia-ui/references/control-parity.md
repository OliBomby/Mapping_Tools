...
| MaterialDesignInXaml `PopupBox` with a `PackIcon` circle and `ColorPicker` content | Avalonia `ColorPicker`                                          | Use the application-level compatibility theme in `Mapping_Tools.Desktop/Resources/Styles/ColorPickers.axaml`. Keep the feature-owned color binding and converter, but do not reproduce the legacy popup or set picker dimensions in the view. The shared theme supplies the 35-pixel colored-circle toggle, the full built-in `ColorView` flyout, Material palette roles, compact outlined component editors, centered input text, and full RGB/HSV hit targets.                               |
...
| MaterialDesignInXaml `MaterialDesignFloatingActionButton` family                  | Material.Avalonia `FloatingButton` with composable classes       | Map `MaterialDesignFloatingActionButton` to an unclassed `FloatingButton`, `MaterialDesignFloatingActionLightButton` to `Classes="light"`, and `MaterialDesignFloatingActionDarkButton` to `Classes="dark"`. Add `mini` for the corresponding mini variants: `Classes="mini"`, `Classes="mini light"`, and `Classes="mini dark"`. The shared `Buttons.axaml` override supplies 40-pixel circular mini geometry plus the primary light/dark palette roles. Keep commands, icons, tooltips, margins, alignment, and enabled state on the view instance. Secondary/accent WPF variants are separate palette roles and must not be silently mapped to primary `light` or `dark`. |
| `ToggleButton` with style `MaterialDesignSwitchToggleButton`                       | `ToggleSwitch`                                                  |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| WPF Geometry Dashboard `HotkeyEditorControl`                                      | `controls:GeometryHotkeyEditor`                                | Retains the numeric Core key/modifier contract, read-only display, modifier-only suppression, and delete/backspace/escape clearing. |
...
| `materialDesign:Card`                                                              | `materialDesign:Card`                                           | Needs this import `xmlns:materialDesign="clr-namespace:Material.Styles.Controls;assembly=Material.Styles"`                                                                                                                                                                                                                                                                                                                                                                                     |
...
- Reference `Avalonia.Controls.ColorPicker` at exactly the same version as the
  other Avalonia packages. Include its Simple theme before
  `Resources/Styles/ColorPickers.axaml` in `App.axaml`; Material.Avalonia
  3.17.0 does not ship a theme for Avalonia's color-picker package.
- Replace the legacy `PopupBox` plus circle toggle plus embedded picker with a
  bare Avalonia `ColorPicker`. Preserve the feature's two-way `Color` binding
  and model converter. Do not add a second flyout, duplicate hex field, or
  rebuild the `ColorView`; Avalonia's `ColorPicker` already owns the dropdown
  and retains its spectrum, palette, components, hex, alpha, and preview UI.
- Let the shared theme own the collapsed 35-pixel circle and internal dropdown
  template. Avalonia 12.1's Simple template places a local right padding of 10
  pixels on that dropdown for its arrow; ordinary style setters cannot remove
  the resulting five-pixel content offset.
- Keep Simple-theme compatibility aliases in a resource dictionary as
  `StaticResource` aliases to Material brush instances. Never place a
  `DynamicResource` markup extension directly in a resource dictionary as the
  alias value: it is not an `IBrush`, produces IDE diagnostics, and crashes
  when the popup enters the logical tree. Use `DynamicResource` normally in
  setters and control-property values.
- Keep the hex `TextBox`, component `NumericUpDown` controls, RGB/HSV radio
  buttons, and their template-part selectors centralized in
