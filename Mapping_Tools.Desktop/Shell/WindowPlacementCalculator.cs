...
public static class WindowPlacementCalculator
{
        ...
        WindowBounds defaultBounds)
    {
        ...
        if (screens.Count == 0) return defaultBounds;

        var candidate = IsUsable(saved) ? saved! : defaultBounds;
        var screen = screens
            .OrderByDescending(area => IntersectionArea(candidate, area))
            .First();
        if (IntersectionArea(candidate, screen) <= 0) screen = screens.FirstOrDefault(area => area.IsPrimary) ?? screens[0];
    ...
    {
        return bounds is { Width: >= 1, Height: >= 1 } && double.IsFinite(bounds.X) && double.IsFinite(bounds.Y) && double.IsFinite(bounds.Width) && double.IsFinite(bounds.Height);
    }
    ...
    private static double IntersectionArea(WindowBounds bounds, DesktopWorkingArea screen)
    {
        double width = Math.Max(
            0,
            Math.Min(bounds.X + bounds.Width, screen.X + screen.Width) - Math.Max(bounds.X, screen.X));
        double height = Math.Max(
            0,
            Math.Min(bounds.Y + bounds.Height, screen.Y + screen.Height) - Math.Max(bounds.Y, screen.Y));
        return width * height;
    }
}
