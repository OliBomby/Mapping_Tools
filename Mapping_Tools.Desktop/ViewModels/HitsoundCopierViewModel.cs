    ...
    IQuickRun
{
    ...
    [ObservableProperty] public partial bool CopyHitsounds { get; set; } = true;
    /// <summary>Gets or sets whether slider-body hitsounds are copied.</summary>
    [ObservableProperty] public partial bool CopyBodyHitsounds { get; set; } = true;
    /// <summary>Gets or sets whether sample sets and custom indices are copied.</summary>
    [ObservableProperty] public partial bool CopySampleSets { get; set; } = true;
    /// <summary>Gets or sets whether timing-point volumes are copied.</summary>
    [ObservableProperty] public partial bool CopyVolumes { get; set; } = true;
    /// <summary>Gets or sets whether target five-percent volumes are protected.</summary>
    [ObservableProperty] public partial bool AlwaysPreserve5Volume { get; set; } = true;
    /// <summary>Gets or sets whether storyboard samples are copied.</summary>
    [ObservableProperty] public partial bool CopyStoryboardedSamples { get; set; }
    /// <summary>Gets or sets whether hitsound-satisfied storyboard samples are skipped.</summary>
    [ObservableProperty] public partial bool IgnoreHitsoundSatisfiedSamples { get; set; } = true;
    /// <summary>Gets or sets whether any target hitsound suppresses a storyboard sample.</summary>
    [ObservableProperty] public partial bool IgnoreWheneverHitsound { get; set; }
    /// <summary>Gets or sets whether unmatched hitsounds target slider ticks.</summary>
    [ObservableProperty] public partial bool CopyToSliderTicks { get; set; }
    /// <summary>Gets or sets whether unmatched hitsounds target slider slides.</summary>
    [ObservableProperty] public partial bool CopyToSliderSlides { get; set; }
    ...
    [ObservableProperty] public partial int StartIndex { get; set; } = 100;
    /// <summary>Gets or sets whether eligible slider ends are muted.</summary>
    [ObservableProperty] public partial bool MuteSliderends { get; set; }
    /// <summary>Gets or sets all accepted beat divisors for the muting filter.</summary>
    [ObservableProperty] public partial IBeatDivisor[] BeatDivisors { get; set; } =
        RationalBeatDivisor.GetDefaultBeatDivisors();
    /// <summary>Gets or sets muted beat divisors for the muting filter.</summary>
    [ObservableProperty] public partial IBeatDivisor[] MutedDivisors { get; set; } =
    ...
    [ObservableProperty] public partial int MutedIndex { get; set; } = -1;
    /// <summary>Gets or sets the muted sample family.</summary>
    [ObservableProperty] public partial SampleSet MutedSampleSet { get; set; } = SampleSet.None;
    ...
    [ObservableProperty] public partial string ResultSummary { get; private set; } = string.Empty;
    ...
    partial void OnCopyToSliderTicksChanged(bool value) => OnPropertyChanged(nameof(StartIndexBoxVisible));
    partial void OnCopyToSliderSlidesChanged(bool value) => OnPropertyChanged(nameof(StartIndexBoxVisible));

    /// <summary>Fetches the current osu! map into the source field.</summary>
    [RelayCommand]
    private async Task ImportLoadAsync() => await SetCurrentPathAsync(path => PathFrom = path, "source");

    /// <summary>Fetches the current osu! map into the target field.</summary>
    [RelayCommand]
    private async Task ExportLoadAsync() => await SetCurrentPathAsync(path => PathTo = path, "target");

    /// <summary>Opens a single-map source picker.</summary>
    [RelayCommand]
    private async Task ImportBrowseAsync() => await PickAsync(
        "Copy hitsounds from", PathFrom, false, paths => PathFrom = paths[0]);

    /// <summary>Opens a multi-map target picker.</summary>
    [RelayCommand]
    private async Task ExportBrowseAsync() => await PickAsync(
    ...
    protected override bool PrepareRun()
    {
        ...
        if (HasErrors || string.IsNullOrWhiteSpace(PathTo))
        {
            return false;
        }
    ...
    /// <inheritdoc/>
    protected override async Task RunCoreAsync() => await RunOptionsAsync(Snapshot(), quick: false);
    ...
    public async Task RunQuickAsync(CancellationToken cancellationToken)
    {
        ...
        options.PathTo = path;
        await RunWithStateAsync(() => RunOptionsAsync(options, quick: true));
    }
    ...
    {
        if (project is not HitsoundCopierProject typed)
        {
    ...
    private HitsoundCopierProject Snapshot() => new()
    {
        PathFrom = PathFrom,
        PathTo = PathTo,
        CopyMode = CopyMode,
    ...
    {
        if (!Enum.IsDefined(project.SourceSelectionMode) || project.BeatDivisors is null ||
            project.BeatDivisors.Length == 0 || project.MutedDivisors is null ||
            project.MutedDivisors.Length == 0 || project.CopyMode is not 0 and not 1)
        {
            throw new InvalidDataException("Hitsound Copier project is incomplete.");
        }
    ...
    private async Task RunOptionsAsync(HitsoundCopierOptions options, bool quick)
    {
        await Execution.ExecuteAsync(
            new ToolExecutionRequest<HitsoundCopierResult>(
                ...
                async context =>
                {
                    HitsoundCopierResult result = await _copier.CopyAsync(
                        options,
                    ...
                    return new ToolExecutionOutput<HitsoundCopierResult>(
                        result,
                        quick ? null : $"Successfully copied hitsounds to {result.ProcessedCount} " +
    ...
    private Task PublishFailureAsync(string title, Exception exception) => _notifications.PublishAsync(
