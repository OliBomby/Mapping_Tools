...
public sealed partial class GeometryDashboardPreferencesWindow : Window
{
    ...
    public GeometryDashboardPreferencesWindow() => InitializeComponent();
    private void CloseWindow(object? sender, RoutedEventArgs eventArgs) => Close(null);
    private void DragWindow(object? sender, PointerPressedEventArgs eventArgs) { if (eventArgs.GetCurrentPoint(this).Properties.IsLeftButtonPressed) BeginMoveDrag(eventArgs); }
    private void ToggleMaximizeWindow(object? sender, TappedEventArgs eventArgs) => WindowState = WindowState == WindowState.Maximized ? WindowState.Normal : WindowState.Maximized;
    private void ResizeWindow(object? sender, PointerPressedEventArgs eventArgs) => GeometryDashboardWindowChrome.Resize(this, sender, eventArgs);
}
...
public sealed partial class GeometryDashboardProjectWindow : Window
{
    ...
    public GeometryDashboardProjectWindow() => InitializeComponent();
    private void SaveSlotsSelectionChanged(object? sender, SelectionChangedEventArgs eventArgs)
    {
        if (DataContext is GeometryDashboardProjectSlotsViewModel viewModel &&
...
public sealed partial class GeometryDashboardGeneratorSettingsWindow : Window
{
    ...
    public GeometryDashboardGeneratorSettingsWindow() => InitializeComponent();
    private void PredicatesSelectionChanged(object? sender, SelectionChangedEventArgs eventArgs)
    {
        if (DataContext is GeometryDashboardGeneratorSettingsDialogViewModel viewModel &&
            sender is ListBox listBox)
        {
            viewModel.SetSelectedPredicates(listBox.SelectedItems?.OfType<SelectionPredicate>() ?? []);
        }
    }
    private void CloseWindow(object? sender, RoutedEventArgs eventArgs) => Close(false);
    private void DragWindow(object? sender, PointerPressedEventArgs eventArgs) { if (eventArgs.GetCurrentPoint(this).Properties.IsLeftButtonPressed) BeginMoveDrag(eventArgs); }
    private void ToggleMaximizeWindow(object? sender, TappedEventArgs eventArgs) => WindowState = WindowState == WindowState.Maximized ? WindowState.Normal : WindowState.Maximized;
    private void ResizeWindow(object? sender, PointerPressedEventArgs eventArgs) => GeometryDashboardWindowChrome.Resize(this, sender, eventArgs);
}
